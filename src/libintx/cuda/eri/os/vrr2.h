...
#include "libintx/cuda/api/thread_block.h"
#include <utility>
...
namespace libintx::cuda::os::vrr2 {
    ...
    std::integral_constant<int,T>)
  {
    ...
    // int AX = cartesian::index(A-T)*NX;
    // for (int ax = AX + thread_rank; ax < cartesian::index(AB-T+1)*NX; ax += num_threads) {
    //   int a = ax/NX;
    //   int x = ax%NX;
    //   double v2 = 0;
    //   for (int k = 0; k < K; ++k) {
    //     double Rt = vrr2::Rt<X-T>(Xpc+k*3, orbital[x]);
    ...
    constexpr int TileXs[] = { 1, 3, 3, 5, 5, 3, 4 };
    constexpr int TileX = TileXs[X-T];
    int idx[2] = { thread_rank/TileX, thread_rank%TileX };
    int dim[2] = { num_threads/TileX, TileX };
    if (thread_rank >= TileX*dim[0]) goto end;
    for (int a = cartesian::index(A-T)+idx[0]; a < cartesian::index(AB-T+1); a += dim[0]) {
      for (int x = cartesian::index(X-T)+idx[1]; x < cartesian::index(X-T+1); x += dim[1]) {
        double v2 = 0;
        for (int k = 0; k < K; ++k) {
          double Rt = vrr2::Rt<X-T>(Xpc+k*3, orbital_axis[x]);
          double v1 = V1[a + k*LD1];
          v2 += Rt*v1;
        }
        for (int t = cartesian::index(T); t < cartesian::index(T+1); ++t) {
    ...
    double *V2)
  {
    ...
    static constexpr int NX = ncart(X);
    int AX = cartesian::index(A)*NX;

    __syncthreads();

    for (int ax = AX + thread_rank; ax < cartesian::index(AB+1)*NX; ax += num_threads) {
      int x = ax%NX;
      int a = ax/NX;
      double v = 0;
      auto t = orbital_axis[cartesian::index(X)+x];
      for (int k = 0; k < K; ++k) {
        double rt = vrr2::Rt<X>(Xpc+k*3, t);
        v += rt*V1[a + k*LD1];
      }
      V2[ax] += v;
    }
